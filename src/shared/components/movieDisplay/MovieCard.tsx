...
export const MovieCard = memo<MovieCardProps>(({
  ...
  }, [isLoading, isPlaceholder, isMovieInWishlist, removeMovie, addMovie, movie.id]);

  // Determinar si usar clases movie-card o movie-carousel
  ...
  const containerClassName = useMemo(() => {
    ...
    return `movie-carousel__item ${isPlaceholder ? "movie-carousel__item--placeholder" : ""}`;
  }, [isMovieCardStyle, className, isPlaceholder]);

  const heartIconClassName = useMemo(() => 
    `movie-carousel__heart-icon ${isMovieInWishlist ? "movie-carousel__heart-icon--active" : ""}`,
    [isMovieInWishlist]
  );
  ...
  return (
    ...
    >
      <div className="movie-carousel__card">
        <button
          className="movie-carousel__wishlist"
